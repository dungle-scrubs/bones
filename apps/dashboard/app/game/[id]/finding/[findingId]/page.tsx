...
import { ArrowLeft, CheckCircle, XCircle, Copy, Clock, FileCode, User, Hash, Calendar } from "lucide-react";
...
async function fetchFinding(gameId: string, findingId: string): Promise<Finding | null> {
...
const STATUS_CONFIG: Record<FindingStatus, { label: string; icon: typeof CheckCircle; variant: "valid" | "invalid" | "duplicate" | "pending"; description: string }> = {
  valid: { label: "Valid", icon: CheckCircle, variant: "valid", description: "This finding has been validated as a legitimate issue." },
  false_flag: { label: "False Flag", icon: XCircle, variant: "invalid", description: "This finding was determined to be incorrect or not a real issue." },
  duplicate: { label: "Duplicate", icon: Copy, variant: "duplicate", description: "This finding duplicates another previously submitted issue." },
  pending: { label: "Pending", icon: Clock, variant: "pending", description: "This finding is awaiting review and validation." },
};

const CONFIDENCE_CONFIG: Record<Confidence, { label: string; color: string; description: string }> = {
  high: { label: "High Confidence", color: "text-valid", description: "Strong evidence this is a real, exploitable issue" },
  medium: { label: "Medium Confidence", color: "text-yellow-500", description: "Likely a real issue but may require specific conditions" },
  low: { label: "Low Confidence", color: "text-orange-500", description: "Possible issue but uncertain or edge case" },
};
...
export default function FindingDetailPage({
  ...
  const { data: finding, isLoading, isError, error } = useQuery({
    ...
    return (
      <main className="min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-sm text-muted-foreground font-mono">Loading finding...</span>
        </div>
      </main>
    );
  }
      ...
      <main className="min-h-screen flex items-center justify-center p-6">
        <div className="text-center space-y-4 max-w-md">
          ...
          <p className="text-sm text-muted-foreground font-mono">
            {error instanceof Error ? error.message : "Could not load finding details"}
          </p>
    ...
    <main className="min-h-screen">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className={cn(
          ...
          finding.status === "duplicate" && "border-duplicate/30 bg-duplicate/5",
          ...
          {finding.confidence && (
            <div className="pt-2 border-t border-border/50">
              ...
              <span className={cn("text-xs font-medium", CONFIDENCE_CONFIG[finding.confidence].color)}>
                {CONFIDENCE_CONFIG[finding.confidence].label}
              </span>
              <p className="text-[10px] text-muted-foreground mt-0.5">{CONFIDENCE_CONFIG[finding.confidence].description}</p>
            </div>
          )}

          {finding.status === "duplicate" && finding.duplicateOf && (
            <div className="pt-2 border-t border-border/50">
              <span className="text-xs text-muted-foreground">Duplicate of: </span>
              <Link
          ...
          <div className="border border-border bg-card p-4 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-mono">{formatAgentName(finding.agentId)}</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
            ...
            <div className="space-y-2">
              <div className="flex items-start gap-2 text-sm">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="font-mono text-xs break-all mt-0.5">{finding.filePath}</div>
                </div>
              </div>
              <div className="flex items-center gap-2 text-sm ml-5">
                <span className="text-muted-foreground">Lines:</span>
                <span className="font-mono">{finding.lineStart} - {finding.lineEnd}</span>
              </div>
            </div>
          </div>
        </div>
