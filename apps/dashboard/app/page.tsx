...
import { ArrowRight, Crosshair, Zap, Trophy, Clock, CheckCircle, Play } from "lucide-react";
...
function getPhaseIcon(phase: string, isComplete: boolean) {
  ...
  if (phase === "hunt" || phase === "review") return <Play className="h-3 w-3" />;
    ...
    <main className="min-h-screen flex flex-col items-center p-6 pl-14">
      <div className="w-full max-w-2xl space-y-8 mt-8">
          ...
          {data && data.games.length > 0 && (
            <div className="divide-y divide-border">
              {data.games.map((game) => (
                ...
                >
                  <div className={cn("flex-shrink-0", getPhaseColor(game.phase))}>
                  ...
                  <div className="flex-shrink-0 text-right">
                    <div className={cn("text-xs font-mono", getPhaseColor(game.phase))}>
                      {game.isComplete ? (
                        ...
                        <span>R{game.round} · {game.phase.replace("_", " ")}</span>
                      )}
                    </div>
        ...
        <div className="flex items-center justify-center gap-2 text-[10px] text-muted-foreground">
          ...
          <span>API: </span>
          <code className="font-mono text-muted-foreground/70">localhost:8019</code>
        </div>
      </div>
    </main>
  );
