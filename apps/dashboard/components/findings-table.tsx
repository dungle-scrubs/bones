...
import { CheckCircle, XCircle, Copy, Clock, Filter, ChevronDown, ArrowUpDown, ArrowUp, ArrowDown, ExternalLink } from "lucide-react";
import { cn, formatAgentName } from "@/lib/utils";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuLabel, DropdownMenuSeparator } from "@/components/ui/dropdown-menu";
...
const STATUS_CONFIG: Record<FindingStatus, { label: string; icon: typeof CheckCircle; variant: "valid" | "invalid" | "duplicate" | "pending" }> = {
...
const CONFIDENCE_CONFIG: Record<Confidence, { label: string; className: string }> = {
  ...
  medium: { label: "Med", className: "bg-yellow-500/20 text-yellow-500 border-yellow-500/30" },
  low: { label: "Low", className: "bg-orange-500/20 text-orange-500 border-orange-500/30" },
};
...
function ConfidenceBadge({ confidence }: { confidence: Confidence | null }) {
  ...
  return (
    <span className={cn("text-[10px] px-1.5 py-0.5 border font-medium uppercase tracking-wider", config.className)}>
      {config.label}
    </span>
  );
}
...
export function FindingsTable({ gameId }: FindingsTableProps) {
  ...
  const [statusFilter, setStatusFilter] = useState<Set<FindingStatus>>(new Set(["valid"]));
  ...
  const columns = useMemo<ColumnDef<Finding>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Rnd",
        cell: ({ row }) => (
          <span className="text-muted-foreground tabular-nums">{row.original.round}</span>
        ),
      },
    ],
    []
  ...
  const statusCounts = useMemo(() => {
    if (!data?.findings) return { valid: 0, false_flag: 0, duplicate: 0, pending: 0 };
    return data.findings.reduce(
      (acc, f) => {
        acc[f.status]++;
        return acc;
      },
      { valid: 0, false_flag: 0, duplicate: 0, pending: 0 } as Record<FindingStatus, number>
    );
  }, [data?.findings]);
          ...
          <DropdownMenuCheckboxItem
            ...
            onCheckedChange={() => {
              if (statusFilter.size === 4) {
                ...
                setStatusFilter(new Set(["valid", "false_flag", "duplicate", "pending"]));
              }
            }}
          >
            Show all
            ...
            <TableBody>
              {table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() => router.push(`/game/${gameId}/finding/${row.original.id}`)}
                >
