...
import { Menu, X, Gamepad2, Clock, Play, CheckCircle } from "lucide-react";
...
export function Sidebar() {
  ...
  const getStatusIcon = (phase: string, isComplete: boolean) => {
    if (isComplete) return <CheckCircle className="h-3 w-3 text-muted-foreground" />;
    if (phase === "hunt" || phase === "review") return <Play className="h-3 w-3 text-green-400" />;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-mono text-xs truncate">{game.id}</div>
