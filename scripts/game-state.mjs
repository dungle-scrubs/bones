...
const commands = {
    ...
    'create-game': async (projectUrl, numAgents, targetScore, huntDuration, reviewDuration, categoriesJson) => {
        const db = await getDb();
        const id = randomUUID();
        db.run(`
            INSERT INTO games (id, project_url, num_agents, target_score, hunt_duration, review_duration, categories, phase)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'setup')
        `, [id, projectUrl, parseInt(numAgents), parseInt(targetScore), parseInt(huntDuration), parseInt(reviewDuration), categoriesJson]);
        saveDb(db);
        return {
            id,
            projectUrl,
            numAgents: parseInt(numAgents),
            targetScore: parseInt(targetScore),
            WARNING: "Use orchestrator.mjs for game flow! Run: node orchestrator.mjs start-hunt " + id
        };
    },
    ...
    'start-round': async (gameId) => {
        const db = await getDb();
        const game = singleRow(db.exec('SELECT * FROM games WHERE id = ?', [gameId]));
        ...
        const endsAt = new Date(Date.now() + game.hunt_duration * 1000).toISOString();

        db.run(`UPDATE games SET current_round = ?, phase = 'hunt', phase_ends_at = ? WHERE id = ?`,
            [newRound, endsAt, gameId]);
        saveDb(db);

        return { gameId, round: newRound, phase: 'hunt', endsAt, durationSeconds: game.hunt_duration };
    },

    'finish-hunt': async (gameId, agentId) => {
        const db = await getDb();
        const game = singleRow(db.exec('SELECT phase, current_round, phase_ends_at FROM games WHERE id = ?', [gameId]));
        if (!game) return { error: 'Game not found' };
        if (game.phase !== 'hunt') return { error: 'Not in hunt phase', phase: game.phase };
        ...
        if (endsAt && now < endsAt) {
            ...
            return { error: 'Hunt phase not ended yet', remainingSeconds: remaining, canFinishAt: game.phase_ends_at };
        }

        db.run(`UPDATE agents SET hunt_done_round = ? WHERE game_id = ? AND id = ?`,
    ...
    'check-hunt-complete': async (gameId) => {
        const db = await getDb();
        const game = singleRow(db.exec('SELECT current_round, phase_ends_at FROM games WHERE id = ?', [gameId]));
        if (!game) return { error: 'Game not found' };
        ...
        const incomplete = rowsToObjects(db.exec(`
    ...
    'start-hunt-scoring': async (gameId) => {
        ...
        const game = singleRow(db.exec('SELECT * FROM games WHERE id = ?', [gameId]));
        if (!game) return { error: 'Game not found' };
        if (game.phase !== 'hunt') return { error: 'Not in hunt phase', currentPhase: game.phase };
        ...
        if (incomplete.length > 0) {
            return { error: 'Not all agents finished hunting', pending: incomplete.map(a => a.id) };
        }

        db.run(`UPDATE games SET phase = 'hunt_scoring', phase_ends_at = NULL WHERE id = ?`, [gameId]);
        saveDb(db);
    ...
    'start-review': async (gameId) => {
        ...
        const game = singleRow(db.exec('SELECT * FROM games WHERE id = ?', [gameId]));
        if (!game) return { error: 'Game not found' };
        if (game.phase !== 'hunt_scoring') return { error: 'Not in hunt_scoring phase - must score hunt bugs first', currentPhase: game.phase };

        const endsAt = new Date(Date.now() + game.review_duration * 1000).toISOString();

        db.run(`UPDATE games SET phase = 'review', phase_ends_at = ? WHERE id = ?`, [endsAt, gameId]);
        saveDb(db);

        return { gameId, round: game.current_round, phase: 'review', endsAt, durationSeconds: game.review_duration };
    },
    ...
    'start-review-scoring': async (gameId) => {
        ...
        const game = singleRow(db.exec('SELECT * FROM games WHERE id = ?', [gameId]));
        if (!game) return { error: 'Game not found' };
        if (game.phase !== 'review') return { error: 'Not in review phase', currentPhase: game.phase };
        ...
        `, [gameId, game.current_round]));
        if (incomplete.length > 0) {
            return { error: 'Not all agents finished reviewing', pending: incomplete.map(a => a.id) };
        }

        db.run(`UPDATE games SET phase = 'review_scoring', phase_ends_at = NULL WHERE id = ?`, [gameId]);
        saveDb(db);
    ...
    'finish-review': async (gameId, agentId) => {
        ...
        const game = singleRow(db.exec('SELECT phase, current_round, phase_ends_at FROM games WHERE id = ?', [gameId]));
        if (!game) return { error: 'Game not found' };
        if (game.phase !== 'review') return { error: 'Not in review phase', phase: game.phase };
        ...
        if (endsAt && now < endsAt) {
            const remaining = Math.ceil((endsAt - now) / 1000);
            return { error: 'Review phase not ended yet', remainingSeconds: remaining, canFinishAt: game.phase_ends_at };
        }

        db.run(`UPDATE agents SET review_done_round = ? WHERE game_id = ? AND id = ?`,
            [game.current_round, gameId, agentId]);
        saveDb(db);
    ...
    'submit-bug': async (gameId, agentId, category, description, filePath, lineStart, lineEnd, codeSnippet = null) => {
        const db = await getDb();
        const game = singleRow(db.exec('SELECT phase, current_round FROM games WHERE id = ?', [gameId]));

        if (!game) return { error: 'Game not found', accepted: false };
        if (game.phase !== 'hunt') return { error: 'Can only submit bugs during HUNT phase', accepted: false };
        ...
        db.run(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
        `, [gameId, game.current_round, agentId, category, description, filePath, parseInt(lineStart), parseInt(lineEnd), codeSnippet, patternHash]);

        const bugId = singleRow(db.exec('SELECT last_insert_rowid() as id'))?.id;
        db.run(`UPDATE agents SET bugs_submitted = bugs_submitted + 1 WHERE game_id = ? AND id = ?`, [gameId, agentId]);
        saveDb(db);
    ...
    'submit-dispute': async (gameId, bugId, disputerId, reason) => {
        ...
        if (!game) return { error: 'Game not found', accepted: false };
        if (game.phase !== 'review') return { error: 'Can only submit disputes during REVIEW phase', accepted: false };

        const bug = singleRow(db.exec('SELECT * FROM bugs WHERE id = ?', [parseInt(bugId)]));
        if (!bug) return { error: 'Bug not found', accepted: false };
        if (bug.agent_id === disputerId) return { error: 'Cannot dispute your own bug', accepted: false };

        const existing = singleRow(db.exec(
            'SELECT id FROM disputes WHERE bug_id = ? AND disputer_id = ?',
            [parseInt(bugId), disputerId]
        ));
        if (existing) return { error: 'Already disputed this bug', accepted: false };

        db.run(`
            INSERT INTO disputes (game_id, round_number, bug_id, disputer_id, reason, status)
            VALUES (?, ?, ?, ?, ?, 'pending')
        `, [gameId, game.current_round, parseInt(bugId), disputerId, reason]);
        saveDb(db);

        return { disputeId: singleRow(db.exec('SELECT last_insert_rowid() as id'))?.id, bugId: parseInt(bugId), accepted: true };
    },
    ...
    'validate-bug': async (bugId, verdict, refereeVerdict = '', confidence = 'medium') => {
        const db = await getDb();
        const bug = singleRow(db.exec('SELECT * FROM bugs WHERE id = ?', [parseInt(bugId)]));
        ...
        const game = singleRow(db.exec('SELECT phase FROM games WHERE id = ?', [bug.game_id]));
        ...
        const validConfidence = ['high', 'medium', 'low'].includes(confidence?.toLowerCase())
        ...
        if (duplicate) {
            ...
            db.run(`UPDATE bugs SET status = 'duplicate', duplicate_of = ?, points_awarded = ?, validated_at = datetime('now') WHERE id = ?`,
                [duplicate.id, points, parseInt(bugId)]);
            db.run(`UPDATE agents SET bugs_duplicate = bugs_duplicate + 1, score = score + ? WHERE game_id = ? AND id = ?`,
                [points, bug.game_id, bug.agent_id]);
        } else if (verdict.toUpperCase() === 'VALID') {
            status = 'valid';
            points = POINTS.VALID_BUG;
            db.run(`UPDATE bugs SET status = 'valid', referee_verdict = ?, confidence = ?, points_awarded = ?, validated_at = datetime('now') WHERE id = ?`,
                [refereeVerdict, validConfidence, points, parseInt(bugId)]);
            db.run(`UPDATE agents SET bugs_valid = bugs_valid + 1, score = score + ? WHERE game_id = ? AND id = ?`,
                [points, bug.game_id, bug.agent_id]);
        } else {
            status = 'false_flag';
            points = POINTS.FALSE_POSITIVE;
            db.run(`UPDATE bugs SET status = 'false_flag', referee_verdict = ?, points_awarded = ?, validated_at = datetime('now') WHERE id = ?`,
                [refereeVerdict, points, parseInt(bugId)]);
            db.run(`UPDATE agents SET bugs_false = bugs_false + 1, score = score + ? WHERE game_id = ? AND id = ?`,
                [points, bug.game_id, bug.agent_id]);
        }

        saveDb(db);
        return { bugId: parseInt(bugId), status, points, confidence: status === 'valid' ? validConfidence : null, duplicateOf: duplicate?.id };
    },

    'resolve-dispute': async (disputeId, verdict, refereeVerdict = '') => {
        const db = await getDb();
        const dispute = singleRow(db.exec('SELECT * FROM disputes WHERE id = ?', [parseInt(disputeId)]));
        if (!dispute) return { error: 'Dispute not found' };
        if (dispute.status !== 'pending') return { error: 'Dispute already resolved' };

        // ENFORCE: Can only resolve during review_scoring phase
        const game = singleRow(db.exec('SELECT phase FROM games WHERE id = ?', [dispute.game_id]));
        ...
        const successful = verdict.toUpperCase() === 'SUCCESSFUL' || verdict.toUpperCase() === 'VALID';
        const status = successful ? 'successful' : 'failed';
        const points = successful ? POINTS.DISPUTE_WON : POINTS.DISPUTE_LOST;

        db.run(`UPDATE disputes SET status = ?, referee_verdict = ?, points_awarded = ?, resolved_at = datetime('now') WHERE id = ?`,
            [status, refereeVerdict, points, parseInt(disputeId)]);

        const statField = successful ? 'disputes_won' : 'disputes_lost';
        db.run(`UPDATE agents SET ${statField} = ${statField} + 1, score = score + ? WHERE game_id = ? AND id = ?`,
            [points, dispute.game_id, dispute.disputer_id]);

        saveDb(db);
        return { disputeId: parseInt(disputeId), status, points };
    },
    ...
    'check-winner': async (gameId) => {
        const db = await getDb();
        const game = singleRow(db.exec('SELECT target_score FROM games WHERE id = ?', [gameId]));
        if (!game) return { error: 'Game not found' };

        const agents = rowsToObjects(db.exec(
        ...
        if (atTarget.length === 0) {
            return { winner: null, reason: 'No agent reached target', continueGame: true };
        }

        if (atTarget.length === 1) {
            return { winner: atTarget[0].id, score: atTarget[0].score, reason: 'Reached target', continueGame: false };
        }
        ...
        if (tied.length === 1) {
            return { winner: tied[0].id, score: tied[0].score, reason: 'Highest score above target', continueGame: false };
        }
    ...
    'complete-game': async (gameId, winnerId) => {
        const db = await getDb();
        db.run(`UPDATE games SET phase = 'complete', winner_agent_id = ?, completed_at = datetime('now') WHERE id = ?`,
            [winnerId, gameId]);
        db.run(`UPDATE agents SET status = 'winner' WHERE game_id = ? AND id = ?`, [gameId, winnerId]);
        saveDb(db);

        const agents = rowsToObjects(db.exec('SELECT * FROM agents WHERE game_id = ? ORDER BY score DESC', [gameId]));
    ...
    'game-state': async (gameId) => {
        ...
        const agents = rowsToObjects(db.exec('SELECT * FROM agents WHERE game_id = ? ORDER BY score DESC', [gameId]));
        const bugs = rowsToObjects(db.exec('SELECT * FROM bugs WHERE game_id = ?', [gameId]));
        const disputes = rowsToObjects(db.exec('SELECT * FROM disputes WHERE game_id = ?', [gameId]));
    ...
    'active-game': async (projectUrl) => {
        const db = await getDb();
        const game = singleRow(db.exec(`
            SELECT * FROM games WHERE project_url = ? AND phase != 'complete' ORDER BY created_at DESC LIMIT 1
        `, [projectUrl]));
        if (game) {
            return {
                ...game,
                WARNING: "Use orchestrator.mjs for game flow! Run: node orchestrator.mjs status " + game.id
            };
        }
        return { active: false, TIP: "To start a new game, run: node orchestrator.mjs setup <project_url>" };
    },

    'heartbeat': async (gameId, agentId) => {
        const db = await getDb();
        db.run(`UPDATE agents SET last_heartbeat = datetime('now') WHERE game_id = ? AND id = ?`, [gameId, agentId]);
        saveDb(db);
